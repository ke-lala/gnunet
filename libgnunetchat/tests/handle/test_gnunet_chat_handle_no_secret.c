 ...
 * @file test_gnunet_chat_handle_no_secret.c
 */

#include "test_gnunet_chat.h"

#define TEST_NO_SECRET_ID     "gnunet_chat_handle_no_secret"

enum GNUNET_GenericReturnValue
on_gnunet_chat_handle_no_secret_msg(void *cls,
                                    struct GNUNET_CHAT_Context *context,
                                    struct GNUNET_CHAT_Message *message)
{
  static unsigned int secret_stage = 0;
  ...
  switch (GNUNET_CHAT_message_get_kind(message))
  {
    ...
    case GNUNET_CHAT_KIND_REFRESH:
      ...
      if (secret_stage == 0)
      {
        account = GNUNET_CHAT_find_account(handle, TEST_NO_SECRET_ID);
    ...
    case GNUNET_CHAT_KIND_LOGIN:
      ...
      ck_assert_uint_eq(secret_stage, 1);

      name = GNUNET_CHAT_account_get_name(account);

      ck_assert_ptr_nonnull(name);
      ck_assert_str_eq(name, TEST_NO_SECRET_ID);
    ...
    case GNUNET_CHAT_KIND_LOGOUT:
      ...
      ck_assert_ptr_nonnull(account);
      ck_assert_ptr_eq(connected, account);
      ck_assert_uint_eq(secret_stage, 2);
...
REQUIRE_GNUNET_CHAT_ACCOUNT(gnunet_chat_handle_connection, TEST_NO_SECRET_ID)
...
call_gnunet_chat_handle_connection(const struct GNUNET_CONFIGURATION_Handle *cfg)
{
  ...
  handle = GNUNET_CHAT_start(cfg, on_gnunet_chat_handle_no_secret_msg, &handle);
...
START_SUITE(handle_suite, "Handle (no secret)")
