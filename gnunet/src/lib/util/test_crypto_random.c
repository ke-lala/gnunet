...
test (void)
{
  ...
  for (i = 0; i < 1024; i++)
    GNUNET_break (1024 > (buf[i] = GNUNET_CRYPTO_random_u32 (1024)));
  for (i = 0; i < 10; i++)
  {
    b2 = GNUNET_CRYPTO_random_permute (1024);
  ...
  for (n = 10; n < 1024LL * 1024LL * 1024LL; n *= 10)
    GNUNET_break (n > GNUNET_CRYPTO_random_u64 (n));
  GNUNET_CRYPTO_random_timeflake (&tf);
  return 0;
}
