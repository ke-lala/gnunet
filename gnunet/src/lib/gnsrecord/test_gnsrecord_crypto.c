...
static void
test_with_type (struct GNUNET_CRYPTO_BlindablePrivateKey *privkey)
{
  ...
  GNUNET_GNSRECORD_query_from_public_key (&pubkey,
                                          "testlabel",
                                          &query_pub);
  ec = GNUNET_GNSRECORD_encryption_context_setup_owner (privkey);
  ...
  /* Create block */
  {
    size_t rdata_len = GNUNET_GNSRECORD_records_get_size (RECORDS, s_rd);
    unsigned char rdata[rdata_len];

    GNUNET_GNSRECORD_record_data_to_rdata (RECORDS,
                                           s_rd,
                                           rdata,
                                           rdata_len);
...
{
  struct GNUNET_CRYPTO_BlindablePrivateKey privkey;
  struct GNUNET_CRYPTO_BlindablePrivateKey privkey_ed;
