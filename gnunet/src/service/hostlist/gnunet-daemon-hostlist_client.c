...
get_bootstrap_server ()
{
  ...
  urls = GNUNET_CRYPTO_random_u32 (urls) + 1;
  pos = strlen (servers) - 1;
  while (pos > 0)
  {
    if (servers[pos] == ' ')
...
download_get_url ()
{
  ...
  index =
    GNUNET_CRYPTO_random_u32 (linked_list_size);
  counter = 0;
...
download_hostlist ()
{
  ...
  if (NULL != proxy)
  {
    ...
    CURL_EASY_SETOPT (curl, CURLOPT_PROXYTYPE, (long) proxy_type);
...
#endif
  CURL_EASY_SETOPT (curl, CURLOPT_BUFFERSIZE, (long) GNUNET_MAX_MESSAGE_SIZE);
