...
static struct GNUNET_CRYPTO_BlindablePublicKey s_zone_value;
     ...
     struct GNUNET_TESTING_Peer *peer)
{
  ...
  privkey.type = htonl (GNUNET_GNSRECORD_TYPE_EDKEY);
  GNUNET_CRYPTO_eddsa_key_create (&privkey.eddsa_key);
  /* get public key */
  GNUNET_CRYPTO_blindable_key_get_public (&privkey,
                                          &pubkey);

  GNUNET_CRYPTO_random_block (&s_zone_value,
                              sizeof(s_zone_value));
  s_zone_value.type = htonl (GNUNET_GNSRECORD_TYPE_EDKEY);
  {
