...
delayed_get (void *cls)
{
  ...
  while (1)
  {
    r = GNUNET_CRYPTO_random_u32 (n_active);
    get_ac = &a_ac[r];
    if (NULL != get_ac->hash)
      break;
  }
  get_ac->nrefs++;
  ac->get_ac = get_ac;
  r = GNUNET_CRYPTO_random_u32 (num_puts_per_peer);
...
delayed_put (void *cls)
{
  ...
  block_size += GNUNET_CRYPTO_random_u32 (
    GNUNET_CONSTANTS_MAX_ENCRYPTED_MESSAGE_SIZE
    - 512);
  GNUNET_CRYPTO_random_block (block,
                              block_size);
  ac->put_count--;
  ...
  case MODE_PUT:
    {
      ...
      peer_delay_put.rel_value_us =
        GNUNET_CRYPTO_random_u64 (delay_put.rel_value_us);
  ...
  case MODE_GET:
    {
      ...
      peer_delay_get.rel_value_us =
        ...
        + GNUNET_CRYPTO_random_u64 (delay_get.rel_value_us);
          ...
          unsigned int links_failed)
{
  ...
  {
    if (GNUNET_CRYPTO_random_u32 (100) >= put_probability)
      continue;
