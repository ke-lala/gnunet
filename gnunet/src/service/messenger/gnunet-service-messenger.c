                   ...
                   const struct GNUNET_MESSENGER_RoomMessage *msg)
{
  ...
  if ((! GNUNET_is_zero (&(msg->door))) ||
      (GNUNET_OK != get_service_peer_identity (messenger, &door)))
    GNUNET_memcpy (&door, &(msg->door), sizeof (door));

  GNUNET_log (GNUNET_ERROR_TYPE_DEBUG, "Entering room: %s, %s\n", GNUNET_h2s (
                &(msg->key)), GNUNET_i2s (&door));

  if (GNUNET_YES == entry_srv_handle_room (msg_client->handle, &door, &(msg->key
                                                                        )))
  {
    sync_srv_handle_room (msg_client->handle,
                          GNUNET_MESSAGE_TYPE_MESSENGER_ROOM_ENTRY,
                          &(msg->key), &(msg->previous), &(msg->epoch), &door);
  }
  else
