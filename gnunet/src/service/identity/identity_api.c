...
GNUNET_IDENTITY_ego_get_anonymous ()
{
  ...
  if (setup)
    return &anon;
  anon.pk.type = htonl (GNUNET_PUBLIC_KEY_TYPE_EDDSA);
  anon.pub.type = htonl (GNUNET_PUBLIC_KEY_TYPE_EDDSA);
  anon.pk.eddsa_key = *GNUNET_CRYPTO_eddsa_key_get_anonymous ();
...
private_key_create (enum GNUNET_CRYPTO_BlindableKeyType ktype,
...
GNUNET_IDENTITY_create (struct GNUNET_IDENTITY_Handle *h,
                        ...
                        enum GNUNET_CRYPTO_BlindableKeyType ktype,
