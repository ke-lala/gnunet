 ...
 */
static void
t_ax_hmac_hash (const struct GNUNET_CRYPTO_AeadSecretKey *key,
 ...
 */
static void
t_hmac_derive_key (const struct GNUNET_CRYPTO_AeadSecretKey *key,
              ...
              size_t size)
{
  ...
  {
    /* Advance ratchet */
    struct GNUNET_CRYPTO_AeadSecretKey keys[3];
  ...
  mac = (struct GNUNET_CRYPTO_AeadMac*) &((char*) dst)[size];
  GNUNET_assert (GNUNET_OK == GNUNET_CRYPTO_aead_encrypt (size,
                                                          src,
                                                          0,
                                                          NULL,
                                                          &MK,
              ...
              size_t size)
{
  ...
  struct GNUNET_CRYPTO_AeadNonce iv;
  struct GNUNET_CRYPTO_AeadMac *mac;
  size_t macoffset = size - sizeof (struct GNUNET_CRYPTO_AeadMac);
  ...
  mac = (struct GNUNET_CRYPTO_AeadMac*) &((char*) src)[macoffset];
  GNUNET_assert (GNUNET_OK == GNUNET_CRYPTO_aead_decrypt (size,
                                                          src,
                                                          0,
                                                          NULL,
                                                          &MK,
             ...
             const unsigned char *additional_data)
{
  ...
  GNUNET_CRYPTO_hkdf_gnunet (&nonce,
                             sizeof nonce,
                             NULL,
                             0,
                             &ax->HKs,
                             sizeof ax->HKs);
  GNUNET_assert (GNUNET_OK == GNUNET_CRYPTO_aead_encrypt (sizeof(msg->ax_header)
                                                          ,
                                                          (unsigned char*) &msg
                                                          ->ax_header,
 ...
 *
 * @param hkr key material to use.
 * @param src Message whose header to decrypt.
 * @param esize size of encrypted payload after message
 * @param dst Where to decrypt header to.
 * @return GNUNET_OK if MAC was validated successfully
 */
static enum GNUNET_GenericReturnValue
t_h_decrypt (const struct GNUNET_CRYPTO_AeadSecretKey *hkr,
             ...
             struct GNUNET_CADET_TunnelEncryptedMessage *dst)
{
  ...
  return GNUNET_CRYPTO_aead_decrypt (sizeof(struct GNUNET_CADET_AxHeader),
                                     (unsigned char*) &src->ax_header,
                                     esize,
                                     (unsigned char*) &src[1],
                                     hkr,
                                     &iv,
                 ...
                 size_t size)
{
  ...
  struct GNUNET_CRYPTO_AeadSecretKey *valid_HK;
  size_t esize;
  size_t len;
  unsigned int N;
  size_t macoffset = size - sizeof (struct GNUNET_CRYPTO_AeadMac);
  ...
  {
    /* Try decrypt header */
    if (GNUNET_OK == t_h_decrypt (&key->HK,
                                  src,
                                  esize,
                                  &plaintext_header))
    {
  ...
  if (0 != GNUNET_CRYPTO_aead_decrypt (
        len,
        (unsigned char*) &src[1],
        0,
        NULL,
        &key->MK,
        &iv,
        (struct GNUNET_CRYPTO_AeadMac*) &src[macoffset],
        dst))
  {
    return -1;
  }
  delete_skipped_key (ax,
                      key);
  return len - sizeof (struct GNUNET_CRYPTO_AeadMac);
}
                           ...
                           size_t size)
{
  ...
  if (GNUNET_OK == t_h_decrypt (&ax->HKr,
                                src,
                                esize,
                                &plaintext_header))
  {
  ...
  {
    static const char ctx[] = "axolotl ratchet";
    struct GNUNET_CRYPTO_AeadSecretKey keys[3];   /* RKp, NHKp, CKp */
    struct GNUNET_CRYPTO_AeadSecretKey HK;
    struct GNUNET_HashCode dh;
    struct GNUNET_CRYPTO_EcdhePublicKey *DHRp;

    /* Try Next HK */
    if (GNUNET_OK == t_h_decrypt (&ax->NHKr,
                                  src,
                                  esize,
                                  &plaintext_header))
    {
  ...
  t_ax_decrypt (ax,
                ...
                esize);
  ax->Nr = Np + 1;
  return esize - sizeof (struct GNUNET_CRYPTO_AeadMac);
}
                 ...
                 const struct GNUNET_HashCode *key_result)
{
  ...
  struct GNUNET_CRYPTO_AeadSecretKey keys[5];
  struct CadetTunnelAxolotl *ax;
