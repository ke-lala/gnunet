...
get_random_literal ()
{
  ...
  ridx =
    GNUNET_CRYPTO_random_u32 ((uint32_t) strlen (ALLOWED_LITERALS));
...
REGEX_TEST_generate_random_regex (size_t rx_length, char *matching_str)
{
  ...
  {
    char_op_switch = GNUNET_CRYPTO_random_u32 (2);

    if ((0 == char_op_switch) && ! last_was_op)
    {
      last_was_op = 1;
      rx_op = GNUNET_CRYPTO_random_u32 (4);
...
REGEX_TEST_generate_random_string (size_t max_len)
{
  ...
  len = (size_t) GNUNET_CRYPTO_random_u32 (max_len);
