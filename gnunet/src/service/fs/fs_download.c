                        ...
                        int do_store)
{
  ...
  char enc[len + sizeof (struct GNUNET_CRYPTO_AeadMac)];
  struct GNUNET_CRYPTO_AeadSecretKey sk;
  struct GNUNET_CRYPTO_AeadNonce iv;
  struct GNUNET_HashCode query;

  GNUNET_FS_hash_to_symmetric_key (&chk->key, &sk, &iv);
  if (GNUNET_OK !=
      GNUNET_CRYPTO_aead_encrypt (len,
                                  (unsigned char*) block,
                                  0,
                                  NULL,
                                  &sk,
                                  &iv,
                                  enc,
                                  (struct GNUNET_CRYPTO_AeadMac*) (enc + len)))
  {
    GNUNET_break (0);
    return GNUNET_SYSERR;
  }
  GNUNET_CRYPTO_hash (enc, sizeof enc, &query);
                 ...
                 size_t data_len)
{
  struct GNUNET_FS_ProgressInfo pi;
  unsigned int i;
  char enc[DBLOCK_SIZE + sizeof (struct GNUNET_CRYPTO_AeadMac)];
  ...
  GNUNET_FS_hash_to_symmetric_key (&in_chk.key, &sk, &iv);
  if (GNUNET_OK !=
      GNUNET_CRYPTO_aead_encrypt (dlen,
                                  (unsigned char*) &data[dr->offset],
                                  0,
                                  NULL,
                                  &sk,
                                  &iv,
                                  enc,
                                  (struct GNUNET_CRYPTO_AeadMac*) (enc + dlen)))
  {
    GNUNET_break (0);
    return;
  }
  GNUNET_CRYPTO_hash (enc,
                      dlen + sizeof (struct GNUNET_CRYPTO_AeadMac),
                      &in_chk.query);
  switch (dr->state)
  {
  case BRS_INIT:
                             ...
                             void *value)
{
  ...
  if (prc->size < sizeof (struct GNUNET_CRYPTO_AeadMac))
  {
    GNUNET_break (0);
    dc->emsg = GNUNET_strdup (_ ("invalid message size"));
    goto signal_error;
  }
  bs = GNUNET_FS_tree_calculate_block_size (GNUNET_ntohll (
                                            ...
                                            dr->depth);
  {
    char pt[prc->size - sizeof (struct GNUNET_CRYPTO_AeadMac)];
    if (sizeof pt != bs)
    {
      GNUNET_asprintf (
        &dc->emsg,
    ...
    GNUNET_FS_hash_to_symmetric_key (&dr->chk.key, &skey, &iv);
    if (GNUNET_OK !=
        GNUNET_CRYPTO_aead_decrypt (prc->size,
                                    prc->data,
                                    0,
                                    NULL,
                                    &skey,
