...
GSF_pending_request_get_message_ (struct GSF_PendingRequest *pr)
{
  ...
  if (do_route)
    prio = GNUNET_CRYPTO_random_u32 (pr->public_data.priority + 1);
  else
    prio = 0;
  ...
  if ((GNUNET_NO == success) && (GNUNET_NO == pmc->requested) && (NULL != cp))
  {
    ...
    else
    {
      ...
      mig_pause.rel_value_us =
        GNUNET_CRYPTO_random_u64 (ppd->migration_delay.rel_value_us);
      ppd->migration_delay =
                ...
                const struct PutMessage *put)
{
  ...
  {
    if (GNUNET_YES != active_to_migration)
      putl = 1.0 + GNUNET_CRYPTO_random_u32 (5);
    block_time = GNUNET_TIME_relative_multiply (
      GNUNET_TIME_UNIT_MILLISECONDS,
      5000 + GNUNET_CRYPTO_random_u32 ((unsigned int) (60000 * putl * putl)));
    GNUNET_log (
      GNUNET_ERROR_TYPE_DEBUG,
