...
GNUNET_FS_search_start_probe_ (struct GNUNET_FS_SearchResult *sr)
{
  ...
  off = len / DBLOCK_SIZE;
  if (off > 0)
    off = GNUNET_CRYPTO_random_u32 (off);
  off *= DBLOCK_SIZE;
...
decrypt_block_with_keyword (const struct GNUNET_FS_SearchContext *sc,
                            const struct GNUNET_CRYPTO_EddsaPublicKey *dpub,
                            const void *edata,
                            size_t edata_size,
                            char *data)
{
  const struct GNUNET_CRYPTO_EddsaPrivateKey *anon;
  struct GNUNET_CRYPTO_EddsaPublicKey anon_pub;
  unsigned int i;
  ...
  /* decrypt */
  anon = GNUNET_CRYPTO_eddsa_key_get_anonymous ();
  GNUNET_CRYPTO_eddsa_key_get_public (anon, &anon_pub);
...
schedule_transmit_search_request (struct GNUNET_FS_SearchContext *sc)
{
         ...
         (GNUNET_YES == first_call))
  {
    ...
    else
    {
      ...
      GNUNET_CRYPTO_eddsa_public_key_derive (&sc->uri->data.sks.ns,
                                             sc->uri->data.sks.identifier,
                                             "fs-ublock",
                                             &dpub);
...
GNUNET_FS_search_start_searching_ (struct GNUNET_FS_SearchContext *sc)
{
  ...
  if (GNUNET_FS_uri_test_ksk (sc->uri))
  {
    ...
    for (i = 0; i < sc->uri->data.ksk.keywordCount; i++)
    {
      ...
      GNUNET_CRYPTO_eddsa_public_key_derive (&anon_pub,
                                             keyword,
                                             "fs-ublock",
                                             &sre->dpub);
