 ...
 */
static void
derive_ublock_encryption_key (struct GNUNET_CRYPTO_AeadSecretKey *skey,
                              ...
                              const struct GNUNET_CRYPTO_EddsaPublicKey *pub)
{
  ...
  GNUNET_FS_hash_to_symmetric_key (&key, skey, iv);
}
                           ...
                           void *output)
{
  ...
  if (input_len < sizeof *mac)
    return;
  derive_ublock_encryption_key (&skey, &iv,
                                label, ns);
  mac = (struct GNUNET_CRYPTO_AeadMac*) (((char*) input)
                                         + input_len
                                         - sizeof *mac);
  GNUNET_CRYPTO_aead_decrypt (input_len - sizeof (*mac),
                              input,
                              0,
                              NULL,
                              &skey,
                           ...
                           GNUNET_FS_UBlockContinuation cont, void *cont_cls)
{
  ...
  struct GNUNET_CRYPTO_AeadNonce iv;
  struct GNUNET_CRYPTO_AeadSecretKey skey;
  struct GNUNET_CRYPTO_EddsaPrivateScalar nsd;
  ...
  size = sizeof(struct UBlock) + slen + mdsize + ulen + sizeof (struct
  ...
  GNUNET_CRYPTO_eddsa_key_get_public (ns,
                                      &pub);
  derive_ublock_encryption_key (&skey, &iv,
                                label, &pub);
  ...
  /* encrypt ublock */
  {
    ...
    mac = (struct GNUNET_CRYPTO_AeadMac*) (((char*) ub_enc) + size - sizeof *mac
                                           );
    GNUNET_CRYPTO_aead_encrypt (ulen + slen + mdsize,
                                (unsigned char*) &ub_plain[1],
                                0,
                                NULL,
                                &skey,
                                &iv,
  ...
  GNUNET_free (ub_plain);
  ub_enc->purpose.size = htonl (size
  ...
  GNUNET_CRYPTO_eddsa_private_key_derive (ns, label, "fs-ublock", &nsd);
  GNUNET_CRYPTO_eddsa_key_get_public_from_scalar (&nsd,
                                                  &ub_enc->verification_key);
  GNUNET_assert (GNUNET_OK ==
                 GNUNET_CRYPTO_eddsa_sign_derived (ns,
                                                   label,
                                                   "fs-ublock",
                                                   &ub_enc->purpose,
                                                   &ub_enc->signature));
