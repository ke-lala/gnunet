...
get_random_peer_from_peermap (struct GNUNET_CONTAINER_MultiPeerMap *valid_peers)
{
  ...
  iterator_cls->index = GNUNET_CRYPTO_random_u32 (
                    ...
                    unsigned int spread)
{
   ...
   */
  rand_delay = GNUNET_CRYPTO_random_u32 (max_rand_delay);
...
do_round (void *cls)
{
  ...
  {
    permut = GNUNET_CRYPTO_random_permute (View_size (sub->view));
  ...
  {   /* If conditions for update are fulfilled, update */
    ...
    permut = GNUNET_CRYPTO_random_permute (CustomPeerMap_size (sub->push_map));
    ...
    permut = GNUNET_CRYPTO_random_permute (CustomPeerMap_size (sub->pull_map));
    for (i = first_border; i < second_border; i++)
    {
      int inserted;
      inserted = insert_in_view (sub,
