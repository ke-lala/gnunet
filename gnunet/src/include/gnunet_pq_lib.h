 ...
 *
 * @param pg database handle
 * @param sql actual SQL text
 * @return #GNUNET_OK on success
 */
enum GNUNET_GenericReturnValue
GNUNET_PQ_prepare_anon (struct GNUNET_PQ_Context *pg,
                        const char *sql);
...
#ifndef GNUNET_PQ_RECONNECT_CALLBACK_CLOSURE
#define GNUNET_PQ_RECONNECT_CALLBACK_CLOSURE void
#endif
 ...
 */
typedef void
(*GNUNET_PQ_ReconnectCallback) (GNUNET_PQ_RECONNECT_CALLBACK_CLOSURE *cls,
 ...
 * @param rc function to call on each connect to the database
 ...
 *
 * @param[in,out] db database to load versioning for
 * @return #GNUNET_OK on success
 *         #GNUNET_NO if it was already loaded
...
/**
 * Check if the database is current with respect to
 * database migrations using @a prefix
 *
 * @param[in,out] db database to check versioning for
 * @param prefix prefix to use to check migrations
 * @return #GNUNET_OK on success
 *         #GNUNET_NO if migrations are pending
 *         #GNUNET_SYSERR on failure
 */
enum GNUNET_GenericReturnValue
 ...
 *
 * @param[in,out] db database connection to reinitialize
 */
void
 ...
 * the full filename.
 *
 * @param[in,out] db database context to use
 * @param buf filename infix (!) with the SQL code to run
 * @return #GNUNET_OK on success
 *         #GNUNET_NO if patch @a buf does not exist
 *         #GNUNET_SYSERR on error
 */
enum GNUNET_GenericReturnValue
 ...
 *
 * @return #GNUNET_OK on success
 *         #GNUNET_NO if no upgrade was needed
 *         #GNUNET_SYSERR if the upgrade failed
 */
enum GNUNET_GenericReturnValue
