...
/**
 * @brief length of the sessionkey in bytes
 */
 ...
 */
#define GNUNET_CRYPTO_AES_IV_LENGTH (128 / 8)
 ...
 */
enum GNUNET_CRYPTO_BlindableKeyType
{
  /**
 ...
 *
 * Consider using AEAD schemes instead of HMAC.
 */
 ...
 *
 * @param max the upper limit (exclusive) for the random number
 * @return a random value in the interval [0,@a i) (exclusive).
 */
uint32_t
GNUNET_CRYPTO_random_u32 (uint32_t i);
...
/**
 * @ingroup crypto
 * Decrypt or encrypt a given block using a symmetric key using AES in counter
 * mode.
 *
 * @param in_buf the data to en/decrypt
 * @param in_buf_len the size if the data in bytes
 * @param key the AES key to use
 * @param iv the initialization vector to use
 * @param out_buf buffer to store the result at
 */
[[ deprecated ("Use GNUNET_CRYPTO_aead_decrypt.") ]]
void
 ...
 * @deprecated
 */
struct GNUNET_CRYPTO_XSalsa20SecretKey
{
  /**
   * 256-bit key.
   */
 ...
 * @deprecated
 */
struct GNUNET_CRYPTO_XSalsa20Nonce
{
  /**
 ...
 * @deprecated
 */
struct GNUNET_CRYPTO_XSalsa20Tag
{
  /**
   * Initialization vector
   */
 ...
 *
 * @param in_buf the data to encrypt
 * @param in_buf_len the size if the data in bytes
 * @param key the key to use
 * @param nonce the initialization vector to use
 * @param out_buf buffer to store the result at
 ...
 * @brief type for session keys
 */
struct GNUNET_CRYPTO_AeadNonce
{
  /**
   * Initialization vector
   */
 ...
 *
 * @param ct_len the size of ct in bytes
 * @param ct the data to decrypt
 * @param aad_len the size of additional data in bytes (may be 0)
 * @param aad the additional data (may be NULL)
 * @param key the key to use
 * @param nonce the initialization vector to use
 * @param mac the mac to use
 * @param pt buffer to store the plaintext result at. Must be at least ct_len
 * @return GNUNET_OK on success, else the mac was invalid
 */
enum GNUNET_GenericReturnValue
 ...
 * Use random, independent keys for each target endpoint (and direction).
 * Use unique nonces for each message.
 *
 * This schemes allows you to choose a random nonce
 * for every encrypted message.
 * (Background: Some schemes require unique nonces, but
 * their nonces are too short to choose them safely
 * randomly (i.e. they may statistically collide).
 *
 *
 * @param pt_len the size if the data in bytes
 * @param pt the data to encrypt
 * @param aad_len the size of additional data in bytes (may be 0)
 * @param aad the additional data (may be NULL)
 * @param key the key to use
 * @param nonce the initialization vector to use
 * @param ct buffer to store the ciphertext result at. Must be at least pt_len.
 * @param mac buffer to store the mac result at
 * @return GNUNET_OK on success
 */
enum GNUNET_GenericReturnValue
 ...
 */
[[ deprecated ("Use EdDSA and its APIs. This ECDSA is not standards compliant.") ]]
void
...
enum GNUNET_CRYPTO_HpkeKem
{
  ...
  GNUNET_CRYPTO_HPKE_KEM_DH_X25519ELLIGATOR_HKDF256 = 0x0022,
};
...
/**
 * Deprecated APIs.
 * APIs the we keep around because they are still used
 * somewhere, and we have to keep the wire format compatible.
 *
