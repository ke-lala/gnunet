  ...
  case MHD_LIB_INFO_FIXED_VERSION_STRING:
    if (sizeof(output_buf->v_version_string) <= output_buf_size)
    {
      ...
      if ((0 == str_len) ||
          (0 != str_buf[str_len]))
      {
        const uint_fast32_t ver_num = MHD_VERSION;
        size_t pos = 0;

        mhd_assert (0 == str_buf[0]);

        pos += mhd_uint32_to_strx ((ver_num >> 24) & 0xFFu,
                                   str_buf + pos,
                                   sizeof(str_buf) - 1 - pos);
        mhd_assert (1 <= pos);
        mhd_assert (2 >= pos);

        str_buf[pos++] = '.';

        pos += mhd_uint32_to_strx ((ver_num >> 16) & 0xFFu,
                                   str_buf + pos,
                                   sizeof(str_buf) - 1 - pos);
        mhd_assert (3 <= pos);
        mhd_assert (5 >= pos);
        ...
        pos += mhd_uint32_to_strx ((ver_num >> 8) & 0xFFu,
                                   str_buf + pos,
                                   sizeof(str_buf) - 1 - pos);
        mhd_assert (5 <= pos);
        mhd_assert (8 >= pos);
        mhd_assert (sizeof(str_buf) > pos);

        str_buf[pos] = 0;
        str_len = pos;
      }
      output_buf->v_version_string.cstr = str_buf;
      output_buf->v_version_string.len = str_len;
      return MHD_SC_OK;
#endif /* ! VERSION */
    }
    return MHD_SC_INFO_GET_BUFF_TOO_SMALL;
