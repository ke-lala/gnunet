...
send_redirect_fixed_rq_target (struct MHD_Connection *restrict c)
{
  ...
  if ( (fixed_uri_len + 200 > c->daemon->conns.cfg.mem_pool_size) ||
       ...
       (NULL ==
        (hdr_line = (char *) malloc (fixed_uri_len + 1 + hdr_prefix_len))) )
  {
    mhd_STREAM_ABORT (c, mhd_CONN_CLOSE_CLIENT_HTTP_ERR_ABORT_CONN, \
 ...
 * @param[out] hdr_value the value of the parsed header (field)
                      ...
                      struct MHD_Stream *restrict s)
{
  ...
  while (i < str_len)
  {
    ...
    if (value_start + value_len < str_len) /* TODO: check seems unnecessary */
                     ...
                     struct MHD_StringNullable *restrict cookie_val)
{
  ...
  cpy = (char *) mhd_stream_alloc_memory (connection,
                                          cookie_val->len + 1);
  if (NULL == cpy)
...
check_and_alloc_buf_for_upload_processing (struct MHD_Connection *restrict c)
{
  ...
  if ((c->rq.cntn.cntn_size >
      ...
      ! mhd_daemon_get_lbuf (c->daemon,
                             (size_t) c->rq.cntn.cntn_size,
                             &(c->rq.cntn.lbuf)))
  {
...
mhd_stream_call_app_request_cb (struct MHD_Connection *restrict c)
{
          ...
          || ! mhd_ACTION_IS_VALID (c->rq.app_act.head_act.act)))
  {
    ...
    case mhd_ACTION_RESPONSE:
      mhd_assert (NULL != c->rq.app_act.head_act.data.response);
      mhd_response_dec_use_count (c->rq.app_act.head_act.data.response);
      break;
  ...
  if (NULL != act)
  {
         ...
         (mhd_UPLOAD_ACTION_CONTINUE == c->rq.app_act.upl_act.act)))
    {
      ...
      case mhd_UPLOAD_ACTION_RESPONSE:
        mhd_assert (NULL != c->rq.app_act.upl_act.data.response);
        mhd_response_dec_use_count (c->rq.app_act.upl_act.data.response);
        break;
      ...
      else
      {
        ...
        { /* Chunk extension */
          ...
          if (';' == buffer_head[i])
          {
            ...
            mhd_assert (1 <= i);
            /* Found LF position at 'i' (buffer_head[i] = '\n'), chunk ends at i+1 */
            if (bare_lf_as_crlf)
              chunk_size_line_len = i + 1; /* Don't care about CR before LF */
            else if ('\r' == buffer_head[i - 1])
              chunk_size_line_len = i + 1; /* Have CR LF, all good */
            /* else: invalid termination, leave chunk_size_line_len at 0 */
          }
          else
 ...
 * the chunk size line.
 * @param c the connection to handle
 * @param chunk_size_line the optional pointer to the partially received
 *                        the current chunk size line.
 *                        Could be not zero-terminated and can contain binary
 *                        zeros.
 *                        Can be NULL.
 * @param chunk_size_line_size the size of the @a chunk_size_line
 */
static void
                      ...
                      bool required)
{
  ...
  rb = (char *) mhd_pool_reallocate (connection->pool,
                                     connection->read_buffer,
                                     connection->read_buffer_size,
                                     new_size);
  if (NULL == rb)
