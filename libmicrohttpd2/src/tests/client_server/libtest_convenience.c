                           ...
                           struct MHD_Daemon *d)
{
  const struct MHD_DaemonOptionAndValue *options
    = (const struct MHD_DaemonOptionAndValue *) cls;
                  ...
                  &r))
    {
      ...
      for (i = 0; i < n; i++)
      {
        ...
        if (0 != (events[i].events & EPOLLIN))
          MHD_FD_STATE_SET_RECV (state);
        if (0 != (events[i].events & EPOLLOUT))
          MHD_FD_STATE_SET_SEND (state);
        if (0 != (events[i].events & (EPOLLERR | EPOLLHUP)) )
          MHD_FD_STATE_SET_EXCEPT (state);
        MHD_daemon_event_update (
          d,
          (struct MHD_EventUpdateContext *) events[i].data.ptr,
          state);
      }
    }
  }
