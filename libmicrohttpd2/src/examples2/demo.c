...
update_directory (void)
{
  ...
  rdc.buf = (char *) malloc (rdc.buf_size);
  if (NULL == rdc.buf)
  {
               ...
               enum MHD_Bool final_data)
{
  ...
  if (-1 == uc->fd)
  {
    if (0 != filename->len)
    {
      ...
      uc->filename = my_strdup (filename->cstr);
    }
    if (NULL == uc->filename)
    {
      fprintf (stderr,
               ...
               uint_fast64_t upload_size)
{
                     ...
                     "/")) )
  {
    ...
    fd = -1;
    buf.st_size = 0; /* Mute compiler warning */
    if ((0 != path->len) &&
        (NULL == strstr (&url[1],
                         "..")) &&
        (0 != url[0]) &&
                     ...
                     "/")) )
  {
    ...
    uc = (struct UploadContext *) malloc (sizeof (struct UploadContext));
    if (NULL == uc)
      ...
      char *const *argv)
{
  ...
  fprintf (stderr,
           "HTTP server running on port %u. Press ENTER to stop the server.\n",
           port);
  (void) getc (stdin);
  MHD_daemon_destroy (d);
